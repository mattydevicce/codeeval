function game_of_life(input){
	...
	var answer_in_arrays = play_ball(input)
	var final_answer = []
	...
	for( var i=0; i<array.length; i++){
		for( var j=0; j<array[i].length; j++){
			...
			// Check if top element or bottom element
				...
				if (top_or_bottom_case(i,j,array) == 'live'){
					new_life[i][j]="*"
				} else {
					new_life[i][j]="."
				}
...
function corner_case(i,j,array) {
	var send = [];
	var doa = array[i][j]

	if (i==0 && j==0){
		send = [[0,0,0],[0,0,array[i][j+1]],[0,array[i+1][j],array[i+1][j+1]]]
		return the_rule_maker(doa, send)
...
function top_or_bottom_case(i,j,array) {
	var send=[];
	var doa = array[i][j]
...
function middle_case(i,j,array) {
